...
def canonicalize_answer_label(text: str) -> str:
    """
    Minimal canonicalization for discrete answer labels so that exact response
    matching is not broken by simple casing/spacing differences.

    This does NOT change the saved raw answers. It is only used for
    response_match calculation.
    """
    raw = safe_text(text)
    if not raw:
        return ""
    ...
    # fallback: compare trimmed original if not a known discrete label
    return raw
...
def main():
    print_header("STEP 11: EVALUATE ODM OUTPUTS")

    ground_truth_path = PROCESSED_DIR / "ground_truth_odm_selected.csv"
    generated_path = GENERATION_DIR / "generated_answers_odm_selected.csv"

    output_dir = EVALUATION_DIR
    output_path = output_dir / "evaluation_odm_selected.csv"
    summary_output_path = output_dir / "evaluation_summary_odm_selected.csv"
    country_summary_output_path = output_dir / "evaluation_summary_by_country_odm_selected.csv"
    ...
    if not generated_path.exists():
        raise FileNotFoundError(
            f"Generated answers file not found: {generated_path}\n"
            f"Please run step10_generate_answers.py first."
        )
    ...
    for _, row in merged_df.iterrows():
        ...
        print(
            f"Evaluated {country} | {question_id} | "
