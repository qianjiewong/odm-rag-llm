...
def canonicalize_final_answer(answer: str, response_scoring=None) -> str:
    ...
    Priority:
    1. If response_scoring contains an official option matching case-insensitively,
       return that exact official option.
    2. Otherwise normalize common labels like yes/no/partially.
    3. Otherwise preserve original text with light capitalization for short labels.
    """
...
def build_style_instruction(dimension: str, question_id: str) -> str:
    dim = safe_text(dimension).lower()
    qid = safe_text(question_id)

    base = """
Write the justification in a style similar to a strong benchmark ground-truth explanation.
...
- Use structured explanation with short numbered sections if helpful.
...
- If a URL is strongly relevant and clearly supported by the evidence, you may include it inline in the explanation.
...
- The explanation should resemble a polished evidence synthesis close to official ground truth.
...
"""
    ...
    if dim == "policy":
        extra = """
For Policy questions, a good structure often is:
1. Direct answer
2. Legal or strategic basis
3. Governance / implementation / operational support
"""
    else:
        extra = """
For Impact questions, a good structure often is:
1. Direct answer
2. Methodology / monitoring / reuse support
3. Concrete evidence of government, social, or economic impact
"""

    if qid in {"P1", "P2", "P4"}:
        extra += """
These questions often benefit from explicit mention of:
- law / decree / directive
- strategy / roadmap
- implementation measures
"""
    if qid in {"I2", "I10", "I12", "I17", "I27"}:
        extra += """
These questions often benefit from explicit mention of:
- methodology or monitoring process
- report or study
- concrete examples of reuse or impact
"""
...
def build_prompt(
    ...
    style_instruction = build_style_instruction(dimension, question_id)
...
- The justification should sound like a human-written evidence synthesis, not a refusal or benchmark description.
...
def remove_inline_source_markers(text: str) -> str:
    """
    Remove inline evidence markers like:
    [S1], (S2), S3, (S2, S3, S4), (S1, S8)
    from the justification body only.

    This is UI-polish cleanup and should not affect scoring or source lists.
    """
    body = safe_text(text)
    if not body:
        return ""
...
def generate_answer_for_question(
    ...
    """
    Reusable single-question generation function.
...
def main():
    print_header("STEP 10: GENERATE ODM ANSWERS")
    ...
    output_path = output_dir / "generated_answers_odm_selected.csv"
    ...
    from pipeline_runner import run_batch_pipeline

    out_df = run_batch_pipeline(
        question_df=target_df,
        top_k=8,
    )

    out_df = out_df.merge(
        target_df[["country", "question_id", "response_scoring"]],
        on=["country", "question_id"],
        how="left"
    )
    ...
    print(f"\nSaved generated answers to: {output_path}")
